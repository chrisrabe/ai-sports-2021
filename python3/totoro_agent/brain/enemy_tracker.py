...
class EnemyTracker:
    def __init__(self):
		"""Enemy Tracker
		Using enemy_ammo etc. even though it's THIS GUYS AMMO. we're not doubling reverse card. Enemy is OUR enemy. Global english, not relative."""
        self.enemy_pos = [0, 0] # Don't need to be called 'enemy pos' -> it's in the enemytracker lol
		self.enemy_ammo = 3
		self.enemy_health = 3 #fix these you bitch.  etc.
    ...
    def update(self, game_state):
		...
		self.enemy_id = game_state[(self.player_id+1)%2] # Could've just done 'not self.player_id' lol
