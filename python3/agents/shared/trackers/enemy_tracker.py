...
from ..utils.util_functions import get_value_map_object, get_surrounding_empty_tiles, get_world_dimension, get_surrounding_tiles, dummy_bomb, get_blast_zone
...
class EnemyTracker:
    ...
    def update(self, game_state):
        ...
        print("Player pos:", game_state['enemy_pos'], "tick:", game_state['tick'], "Length of enem surrounding empty tiles (from enemy_tracker): ",  len(enemy_surrounding_empty_tiles), enemy_surrounding_empty_tiles, print(len(enemy_surrounding_empty_tiles) ==0))
        ...
        if game_state['enemy_pos'] in blast_zone:
            ...
            print("Blast zone: ", blast_zone, game_state['enemy_pos'] in blast_zone, check, enemy_surrounding_empty_tiles)
            if check:
                game_state['enemy_onestep_trapped'] = True
        #(see if I place a bomb right now, would it hit the opponent AND can the opponent not run away? (surrounding enemy tiles, is that in the blast radius?)-> If yes, place).
